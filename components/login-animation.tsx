...
export function LoginAnimation({ isLoading, onComplete }: LoginAnimationProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 animate-fade-in">
      <div className="bg-card rounded-xl p-8 flex flex-col items-center justify-center max-w-sm w-full">
        <div className="flex flex-col items-center space-y-4">
          ...
          <p className="text-lg font-medium text-card-foreground">Выполняется вход...</p>
        </div>
      </div>
    </div>
  )
