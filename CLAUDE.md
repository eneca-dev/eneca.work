...
## 🚨 ОБЯЗАТЕЛЬНО: Работа по списку задач (Markdown Kanban)

**Источник правил:** `.devtool/features/AI-RULES.md` — это главный документ по работе с задачами. При любом расхождении следуй ему.

**Claude работает ТОЛЬКО в рамках зафиксированной задачи, если пользователь явно не просит иного.**

### Где живут задачи

Канбан ведётся **НЕ** в `module.meta.json`, а через индивидуальные `.md`-файлы (по одному на тикет) в папке `.devtool/features/`. Это исключает merge-конфликты при параллельной работе команды. Каждый тикет = YAML-шапка (frontmatter) + тело с описанием.

### Команда и инициалы (для `assignee` и ID)

| Имя (для `assignee`) | Инициалы (для ID) |
|----------------------|-------------------|
...
### Формат ID и имени файла

`[тип]-[инициалы]-[личный номер]` → напр. `feature-DH-01.md`, `bug-VT-02.md`, `task-EZ-10.md`.
- Личный номер сквозной **по человеку** (не по типу): у каждого исполнителя своя нумерация (01, 02, 03…).
- H1-заголовок внутри файла всегда начинается с шифра задачи: `# bug-VT-02 Название`.
- ❌ Запрещены абстрактные сквозные номера вида `feature-809.md` (вызывают конфликты).

### Идентификация исполнителя

- **Никаких выдумок.** Запрещено угадывать `assignee`. Если неизвестно, кто за рулём — **СПРОСИ** имя пользователя.
- Если задача без исполнителя — `assignee: null` и сообщи в чат: «задача создана, нет исполнителя».

### Статусы (поле `status`)

`in-progress` **удалён**, не используем. Разрешено только:
- `"backlog"` — в бэклоге
- `"todo"` — в очереди на выполнение
- `"review"` — код написан, ожидает проверки (PR)
- `"done"` — завершена

**При переводе в `review`** обязательно дописать в тело `.md` два пункта: (1) как именно решена проблема, (2) на что обратить внимание ревьюеру.

### Метаданные

- **`epic`** (строго одно из): `"bug"`, `"feature"`, `"test"`, `"refactor"`, `"chore"`.
- **`labels`** — **обязательно** содержит версию, вычисленную из имени Git-ветки (ветка `v1.5.0` → `["v1.5.0"]`). На «следующую версию» — сам инкременти (`v1.6.0`).
- При создании нового тикета **прочитай соседний `.md`-файл** в `.devtool/features/` и скопируй структуру frontmatter (`id`, `status`, `priority`, `assignee`, `epic`, `dueDate`, `created`, `modified`, `completedAt`, `labels`, `order`), подставив актуальные значения и текущее время. Не выдумывай новые поля.
