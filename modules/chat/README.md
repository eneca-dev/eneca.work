...
Зелёная кнопка в правом нижнем углу — это AI-помощник. Открывается окно чата, где можно общаться с ботом.
...
- 🗑️ **Очистить** — кнопка с круговой стрелкой
- ✨ **Sparkles** — быстрые команды (План на день, Собрать отчёт)
- ⌨️ **Ввод** — Enter отправляет, Shift+Enter новая строка, макс 500 символов

### Логика
- Сообщения сохраняются в **localStorage** (навсегда, макс 10 штук)
- Отправка на `/api/chat/python` → Python AI-агент на `https://ai-bot.eneca.work`
- JWT токен из Supabase для безопасности
- История изолирована по userId
