        ...
        <div className={`grid grid-cols-2 ${isCompact ? 'gap-2' : 'gap-3'}`}>
          {groups.map((group, groupIndex) => (
            <div 
              ...
              `}
            >
              <div className={`text-xs font-medium text-white ${isCompact ? 'mb-0.5' : 'mb-1'}`}>{group.title}</div>
              <div className={`${isCompact ? 'space-y-0.5' : 'space-y-1'}`}>
                {group.items.map((item, index) => (
                  <div key={index} className="text-xs text-gray-400 truncate max-w-full" title={item.entity_name}>
                    {item.entity_name}
                  </div>
                ))}
              </div>
