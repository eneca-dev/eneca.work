      ...
      {isEditing ? (
        <div className="flex items-center gap-1">
          ...
          <button
            onClick={handleSave}
            disabled={isUpdating}
            className="p-0.5 text-green-500 hover:bg-slate-700 rounded"
          >
            {isUpdating ? (
          ...
          <button
            ...
            className="p-0.5 text-slate-500 hover:text-slate-300 hover:bg-slate-700 rounded"
          >
            <X className="w-3 h-3" />
          </button>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          {!isMain && (
            <button
              onClick={onDelete}
              className={cn(
                'p-0.5 text-slate-600 hover:text-red-400',
...
function AddPartForm({ budgetId, existingTypes, onClose }: AddPartFormProps) {
  ...
  return (
    <div className="space-y-2 p-2 bg-slate-800/50 rounded-lg border border-slate-700/50">
      {/* Part type selector */}
      ...
      {/* Percentage input */}
      <div className="flex items-center gap-2">
        <label className="text-[10px] text-slate-400">Процент:</label>
        <input
          type="text"
          value={percentage}
      ...
      <label className="flex items-center gap-2 cursor-pointer">
        <input
          ...
          className="w-3 h-3 rounded border-slate-600 bg-slate-800 text-amber-500 focus:ring-0 focus:ring-offset-0"
        />
        <span className="text-[10px] text-slate-400">Требует согласования</span>
      </label>
      ...
      >
        ...
        <div className="flex items-center justify-between px-3 py-2 border-b border-slate-700/50">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[11px] font-medium text-slate-300">Распределение бюджета</span>
          </div>
          ...
          {isLoading ? (
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center justify-between">
                <span className="text-[10px] text-slate-400">Распределено:</span>
                <span className={cn(
                  'text-[10px] font-mono',
                  isOverAllocated ? 'text-red-400' : 'text-slate-300'
                )}>
                  {totalPercentage}%
                  {isOverAllocated && (
                    <Tooltip>
        ...
        <div className="px-3 py-2 border-t border-slate-700/50">
          <div className="text-[9px] text-slate-500 leading-relaxed">
            <strong className="text-slate-400">Основной</strong> — базовые расходы.{' '}
