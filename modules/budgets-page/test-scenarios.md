...
| 3 | Изменить процент напрямую | Сумма автоматически пересчитывается ⚠️ (поле % не редактируемое для разделов) |

### TC-022: Редактирование бюджета этапа
**Приоритет:** Critical
**Статус:** ❌ Баг BP-021 - конкатенация значений

| Шаг | Действие | Ожидаемый результат |
|-----|----------|---------------------|
| 1 | Найти этап с бюджетом | Отображается текущее значение |
| 2 | Изменить сумму (ввести 5000) | **Ожидание:** 5000. **Факт:** 550,000,000 (значение конкатенируется) |
| 3 | Проверить % от родителя | Показывает 5500% вместо корректного значения |

**Примечание:** Ввод не заменяет существующее значение, а добавляется к нему. При triple-click + ввод работает корректно.

### TC-023: Редактирование бюджета задачи
**Приоритет:** Critical
**Статус:** ✅ Работает (BP-020 исправлен)

| Шаг | Действие | Ожидаемый результат |
|-----|----------|---------------------|
| 1 | Найти задачу (например "твытвы") | Отображается бюджет (может быть 0) |
| 2 | Кликнуть на textbox суммы | Поле редактируемое ✅ |
| 3 | Ввести 500 | Значение отображается, % пересчитывается ✅ |
| 4 | Нажать Tab | Сохраняется в БД ✅ |
| 5 | Обновить страницу | Значение сохранено ✅ |
...
| ID | Приоритет | Краткое описание | Затронутые TC | Статус |
|----|-----------|------------------|---------------|--------|
| BP-012 | Critical | Модалка редактирования проекта не работает | TC-061 | ❌ Open |
| BP-013 | High | Бюджет проекта не создаётся | TC-060 | ❌ Open |
| BP-014 | High | Выбор сложности не работает | TC-051 | ❌ Open |
| BP-017 | Critical | Создание этапа падает с Failed to fetch | TC-040 | ❌ Open |
| BP-018 | Critical | Добавление части бюджета сбрасывает в 0 | TC-032 | ❌ Open |
| BP-019 | High | Основной в модалке показывает 0 | TC-031 | ❌ Open |
| BP-020 | Critical | Бюджет задачи не сохраняется | TC-023 | ✅ Fixed (2026-01-06) |
| BP-021 | Critical | Конкатенация значений при редактировании бюджета этапа | TC-022 | ❌ Open |
