-- ============================================================================
-- Migration: Add CPI (Cost Performance Index) to v_resource_graph
-- Date: 2025-12-20
-- Description: Adds decomposition_item_cpi calculated field for efficiency tracking
--              CPI = EV / Fact = (Plan × Progress / 100) / Actual Hours
-- ============================================================================

-- ============================================================================
-- ROLLBACK VERSION (for reference)
-- To rollback, run this in a new migration file or use the ROLLBACK file
-- ============================================================================
/*
DROP VIEW IF EXISTS public.v_resource_graph;
-- Then recreate without CPI (use previous version from 2025-12-20_add_actual_hours_to_resource_graph.sql)
*/

-- ============================================================================
-- NEW VERSION (with CPI)
-- ============================================================================
...
CREATE VIEW public.v_resource_graph AS
 SELECT p.project_id,
    ...
    -- ========================================================================
    -- Actual hours from work_logs (added in previous migration)
    -- ========================================================================
    COALESCE(wl_agg.total_actual_hours, 0) AS decomposition_item_actual_hours,
    -- ========================================================================
    -- NEW: CPI (Cost Performance Index)
    -- CPI = EV / Actual = (Plan × Progress / 100) / Actual
    -- CPI > 1.0 = efficient (spending less than planned)
    -- CPI = 1.0 = on track
    -- CPI < 1.0 = inefficient (spending more than planned)
    -- ========================================================================
    CASE
      WHEN COALESCE(wl_agg.total_actual_hours, 0) > 0 THEN
        ROUND(
          (di.decomposition_item_planned_hours * di.decomposition_item_progress / 100.0)
          / NULLIF(wl_agg.total_actual_hours, 0),
          3
        )
      ELSE NULL
   ...
   FROM projects p
     ...
     -- ========================================================================
     -- Aggregate work_logs hours by decomposition_item_id
     -- ========================================================================
     LEFT JOIN LATERAL (
       SELECT
         SUM(wl.work_log_hours) AS total_actual_hours
       FROM work_logs wl
...
-- ============================================================================
-- Verification query (run after migration to verify CPI calculation)
-- ============================================================================
-- SELECT
--   decomposition_item_description,
--   decomposition_item_planned_hours as plan,
--   decomposition_item_actual_hours as actual,
--   decomposition_item_progress as progress,
--   decomposition_item_cpi as cpi,
--   CASE
--     WHEN decomposition_item_cpi >= 1.0 THEN '🟢 Efficient'
--     WHEN decomposition_item_cpi >= 0.8 THEN '🟡 Acceptable'
--     WHEN decomposition_item_cpi < 0.8 THEN '🔴 Critical'
--     ELSE 'N/A'
--   END as status
-- FROM v_resource_graph
-- WHERE decomposition_item_actual_hours > 0
-- ORDER BY decomposition_item_cpi ASC NULLS LAST;
