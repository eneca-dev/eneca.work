...
- **Удаление:** Полное удаление пользователя (`user.delete`).
...
- Создание/удаление пользователей (требует `user.create`/`user.delete`).
...
- **users** (id, name, email, avatar_url, position_id, category_id, ...) - Основная информация о пользователе.
...
- Фильтрация и группировка по отделу, команде, категории, дате регистрации
