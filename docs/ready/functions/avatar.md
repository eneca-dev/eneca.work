...
## Последовательная работа системы загрузки и создания аватара

1. **Пользователь инициирует загрузку**
   - Наводит курсор на аватар в интерфейсе (например, в карточке пользователя).
   - Открывается диалоговое окно для загрузки фотографии (`avatar-upload-dialog.tsx`).

2. **Пользователь выбирает фотографию**
   - Выбирает файл JPEG или PNG.
   - В браузере происходит предпросмотр и автоматическая обрезка изображения до квадратного формата 1024×1024 (Canvas API).
   - Пользователь подтверждает согласие на обработку фотографии нейросетью.

3. **Отправка изображения на сервер**
   - После подтверждения пользователь нажимает кнопку загрузки.
   - Компонент формирует FormData с изображением (ключ — `avatar`).
   - В заголовке запроса указывается access_token пользователя (для идентификации).
   - Отправляется POST-запрос на Edge Function `/functions/v1/generate-avatar-from-photo`.

4. **Обработка изображения на Edge Function**
   - Edge Function получает изображение, access_token и user_id (декодирует из токена).
   - Генерирует аватар с помощью нейросети (например, через OpenAI API).
   - Ресайзит и оптимизирует изображение (JPEG ≤ 50 КБ, 512×512).
   - Сохраняет итоговый аватар в Supabase Storage (например, в бакет avatars).
   - Возвращает публичный URL аватара и user_id в ответе.

5. **Обработка результата на клиенте**
   - Клиент получает ответ с URL нового аватара.
   - Обновляет поле avatar в таблице profiles через Supabase (PATCH по user_id).
   - Использует метод updateAvatar из useUserStore для синхронизации состояния.
   - Интерфейс автоматически обновляет аватар пользователя.
   - Пользователь получает toast-уведомление об успешной загрузке.

6. **Ограничения и UX**
   - Повторная загрузка аватара возможна только через 15 минут.
   - В случае ошибки (например, не удалось сгенерировать аватар) пользователю показывается соответствующее уведомление.

7. **Безопасность**
   - Все запросы выполняются только по HTTPS.
   - Edge Function проверяет access_token, но не обновляет профиль напрямую — это делает только клиент.

**Вся цепочка выглядит так:**
Пользователь → [Клиент: предпросмотр, отправка] → Edge Function (обработка, генерация, сохранение) → [Клиент: обновление профиля, UI, уведомление]

---

Этот подход обеспечивает безопасность, прозрачность и удобство для пользователя, а также простоту поддержки и расширения системы.
